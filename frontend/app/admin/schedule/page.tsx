...
export default function SchedulePage() {
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const body = { type: 'schedule', message, internal_name: internalName, pinned, urgent, published };
      if (editing) {
        const res = await adminFetch(`/api/schedule/${editing.id}`, { method: 'PATCH', body: JSON.stringify(body) });
        if (!res.ok) throw new Error();
      } else {
        const res = await adminFetch('/api/schedule', { method: 'POST', body: JSON.stringify(body) });
        if (!res.ok) throw new Error();
      }
      setShowForm(false);
      fetchItems();
  ...
  async function handleDelete() {
    ...
    setDeleting(true);
    try {
      await adminFetch(`/api/schedule/${deleteTarget.id}`, { method: 'DELETE' });
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button onClick={openCreate} disabled={renderDown}
          className="inline-flex items-center gap-2 px-4 py-2.5 bg-emerald-600 text-white text-sm font-medium rounded-xl hover:bg-emerald-700 disabled:opacity-50 transition-colors min-h-[44px]">
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap mb-2">
                    <span className="px-2 py-0.5 bg-emerald-100 text-emerald-700 text-xs font-medium rounded-full">Schedule</span>
                    {!item.published && <span className="px-2 py-0.5 bg-slate-100 text-slate-500 text-xs rounded-full">Draft</span>}
                    {item.urgent && (
                  ...
                  <p className="text-xs text-slate-400 mt-2">by {item.internal_name} · {formatDateTime(item.created_at)}</p>
                </div>
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-md p-6 space-y-4 max-h-[80vh] overflow-y-auto">
            <h2 className="text-lg font-semibold text-slate-900">{editing ? 'Edit' : 'New'} Schedule Entry</h2>
            <form onSubmit={handleSave} className="space-y-4">
              ...
              <div>
                ...
                <input value={internalName} onChange={(e) => setInternalName(e.target.value)} required
                  className="w-full px-3 py-2.5 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="For internal tracking only" />
                <p className="text-xs text-slate-400 mt-1">Never shown publicly</p>
              </div>
              <div className="flex flex-wrap gap-4">
                <label className="flex items-center gap-2 cursor-pointer min-h-[44px]">
                  <input type="checkbox" checked={pinned} onChange={(e) => setPinned(e.target.checked)} className="w-4 h-4 rounded border-slate-300 text-blue-600 focus:ring-blue-500" />
                  <span className="text-sm text-slate-700">Pinned</span>
                </label>
                <label className="flex items-center gap-2 cursor-pointer min-h-[44px]">
                  <input type="checkbox" checked={urgent} onChange={(e) => setUrgent(e.target.checked)} className="w-4 h-4 rounded border-slate-300 text-red-600 focus:ring-red-500" />
                  <span className="text-sm text-slate-700">Urgent</span>
                </label>
                <label className="flex items-center gap-2 cursor-pointer min-h-[44px]">
                  <input type="checkbox" checked={published} onChange={(e) => setPublished(e.target.checked)} className="w-4 h-4 rounded border-slate-300 text-blue-600 focus:ring-blue-500" />
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button type="submit" disabled={saving} className="flex-1 inline-flex items-center justify-center gap-2 px-4 py-2.5 bg-emerald-600 text-white text-sm font-medium rounded-xl hover:bg-emerald-700 disabled:opacity-50 transition-colors min-h-[44px]">
      ...
      {deleteTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-sm p-6 space-y-4">
            <h2 className="text-lg font-semibold text-slate-900">Delete Schedule Entry?</h2>
