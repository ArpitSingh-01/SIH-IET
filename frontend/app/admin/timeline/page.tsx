...
export default function TimelinePage() {
  ...
  function openCreate() {
    setEditing(null);
    setTitle(''); setDescription(''); setEventDate(''); setEventTime(''); setVisible(true);
    setShowForm(true);
  }
  ...
  function openEdit(item: TimelineEntry) {
    ...
    setEventDate(item.event_date); setEventTime(item.event_time || ''); setVisible(item.visible);
    setShowForm(true);
  }
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const body = { title, description: description || null, event_date: eventDate, event_time: eventTime || null, visible, actor: 'admin' };
      if (editing) {
        await adminFetch(`/api/timeline/${editing.id}`, { method: 'PATCH', body: JSON.stringify(body) });
      } else {
        await adminFetch('/api/timeline', { method: 'POST', body: JSON.stringify(body) });
      }
      setShowForm(false);
      fetchItems();
  ...
  async function handleReorder(index: number, direction: 'up' | 'down') {
    ...
    setItems(newItems);
    try {
      await adminFetch('/api/timeline/reorder', {
        method: 'PATCH',
  ...
  async function toggleVisibility(item: TimelineEntry) {
    try {
      await adminFetch(`/api/timeline/${item.id}`, {
        method: 'PATCH',
  ...
  async function handleDelete() {
    ...
    setDeleting(true);
    try {
      await adminFetch(`/api/timeline/${deleteTarget.id}`, { method: 'DELETE' });
            ...
            <div key={item.id} className={`bg-white rounded-xl border border-slate-200 shadow-sm p-4 ${!item.visible ? 'opacity-60' : ''}`}>
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-slate-400 mt-1">{formatEventDate(item.event_date, item.event_time)}</p>
                </div>
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-md p-6 space-y-4 max-h-[80vh] overflow-y-auto">
            <h2 className="text-lg font-semibold text-slate-900">{editing ? 'Edit' : 'New'} Timeline Entry</h2>
      ...
      {deleteTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-sm p-6 space-y-4">
            <h2 className="text-lg font-semibold text-slate-900">Delete Timeline Entry?</h2>
